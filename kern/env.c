...
int env_alloc(struct Env **new, u_int parent_id) {
	...
	if (parent_id != 0) {
		...
		if(envid2env(parent_id, &parent_env, 0) == 0) {
			// 如果父进程存在，则将父进程的cwd复制给子进程
			strcpy(e->env_cwd, parent_env->env_cwd);
			for (int i = 0; i < 10; i++) {
				if (parent_env->env_vars[i].is_env) { // 只继承环境变量(深拷贝)
			...
			// 如果父进程不存在，则将子进程的cwd设置为根目录
			for (int i = 0; i < 10; i++) {
				e->env_vars[i].name[0] = '\0';
				e->env_vars[i].value[0] = '\0';
				e->env_vars[i].is_env = 0;
				e->env_vars[i].is_readonly = 0;
			}
			strcpy(e->env_cwd, "/");
		}
	} else {
